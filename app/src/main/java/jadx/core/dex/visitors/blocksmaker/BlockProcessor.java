...
public class BlockProcessor extends AbstractVisitor {
	...
	private static boolean removeEmptyBlock(final BlockNode block) {
		if (canRemoveBlock(block)) {
			LOG.debug("Removing empty block: {}", block);
			if (block.getSuccessors().size() == 1) {
				final BlockNode successor = block.getSuccessors().get(0);
				block.getPredecessors().forEach(new Consumer<BlockNode>() {
					@Override
					public void accept(BlockNode pred) {
	...
	private static boolean deduplicateBlockInsns(BlockNode block) {
		if (block.contains(AFlag.LOOP_START) || block.contains(AFlag.LOOP_END)) {
			...
			if (predsCount > 1) {
				...
				final int sameInsnCount = getSameLastInsnCount(predecessors);
				if (sameInsnCount > 0) {
					List<InsnNode> insns = getLastInsns(predecessors.get(0), sameInsnCount);
					insertAtStart(block, insns);
					predecessors.forEach(new Consumer<BlockNode>() {
						@Override
						public void accept(BlockNode blockNode) {
							getLastInsns(blockNode, sameInsnCount).clear();
						}
					});
	...
	private static void computeDominators(MethodNode mth) {
		...
		basicBlocks.forEach(new Consumer<BlockNode>() {
			@Override
	...
	private static void computeBlockDF(final MethodNode mth, BlockNode block) {
		if (block.getDomFrontier() != null) {
			return;
		}
		block.getDominatesOn().forEach(new Consumer<BlockNode>() {
			@Override
			public void accept(BlockNode blockNode) {
				computeBlockDF(mth, blockNode);
			}
		});
	...
	private static void markReturnBlocks(final MethodNode mth) {
	...
	private static void markLoops(MethodNode mth) {
		mth.getBasicBlocks().forEach(new Consumer<BlockNode>() {
			@Override
			public void accept(final BlockNode block) {
				// Every successor that dominates its predecessor is a header of a loop,
				// block -> succ is a back edge.
				block.getSuccessors().forEach(new Consumer<BlockNode>() {
					@Override
					public void accept(BlockNode successor) {
	...
	private static void registerLoops(final MethodNode mth) {
		mth.getBasicBlocks().forEach(new Consumer<BlockNode>() {
			@Override
			public void accept(BlockNode block) {
				if (block.contains(AFlag.LOOP_START)){
					block.getAll(AType.LOOP).forEach(new Consumer<LoopInfo>() {
						@Override
						public void accept(LoopInfo loopInfo) {
							mth.registerLoop(loopInfo);
						}
					});
				}
			}
	...
	private static void removeMarkedBlocks(final MethodNode mth) {
		mth.getBasicBlocks().removeIf(new Predicate<BlockNode>() {
			@Override
