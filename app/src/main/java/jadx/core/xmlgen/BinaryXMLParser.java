...
public class BinaryXMLParser extends CommonBinaryParser {
	...
	private void parseNameSpace() throws IOException {
		...
		final int beginPrefix = is.readInt32();
		int beginURI = is.readInt32();
		nsMap.compute(strings[beginURI], new BiFunction<String, String, String>() {
			@Override
			public String apply(String s, String s2) {
				return strings[beginPrefix];
			}
		});
	}
	...
	private void parseNameSpaceEnd() throws IOException {
		...
		int comment = is.readInt32();
		final int endPrefix = is.readInt32();
		int endURI = is.readInt32();
		nsMap.compute(strings[endURI], new BiFunction<String, String, String>() {
			@Override
			public String apply(String s, String s2) {
				return strings[endPrefix];
			}
		});
	}
